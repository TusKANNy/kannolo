...
mod tests {
    ...
    fn merge_reverse_links_appends_all_incoming_when_there_is_room() {
        ...
        let neighbors: Vec<usize> = graph.neighbors(5).collect();
        assert_eq!(
            neighbors,
            vec![4, 6, 1, 2],
...
impl From<Graph> for GrowableGraph {
    fn from(graph: Graph) -> Self {
        ...
        let mut neighbors: Vec<Optioned<u32>> = Vec::with_capacity(n_nodes * max_degree);
...
impl GrowableGraph {
    ...
    pub fn write_links(&mut self, forward: &[usize], local_id: usize) {
        ...
        for (i, &nbr) in forward.iter().enumerate() {
            self.neighbors[start + i] = Optioned::some(nbr as u32);
        }
    ...
    pub fn append_links(&mut self, node: usize, at: usize, ids: impl Iterator<Item = u32>) {
        ...
        for (i, id) in ids.enumerate() {
            self.neighbors[start + i] = Optioned::some(id);
        }
    }
    ...
    pub fn replace_links(&mut self, node: usize, ids: &[usize], old_degree: usize) {
        ...
        for (i, &id) in ids.iter().enumerate() {
            self.neighbors[start + i] = Optioned::some(id as u32);
        }
